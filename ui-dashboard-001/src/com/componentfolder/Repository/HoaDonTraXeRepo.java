...
public class HoaDonTraXeRepo {
    ...
    public ArrayList<HoaDonTraXeViewModel> getlistdata() {
        try {
            ...
            String sql = "select hoadontraxe.mahoadon,hopdong.mahopdong,ngaytra,phiphatsinh,thanhtien,hopdong.makh from hoadontraxe join hopdong on hoadontraxe.mahopdong = hopdong.mahopdong";
...
//                hdtxvm.setVpm(new ViPhamModel(rs.getInt(4), ""));
                hdtxvm.setPhiphatsinh(rs.getDouble(4));
                hdtxvm.setTongtien(rs.getDouble(5));
                hdtxvm.setIdkh(rs.getString(6));
                list.add(hdtxvm);
            }
            return list;
    ...
    public boolean delete(HoaDonTraXeModel hdhtm) {
        try ( Connection conn = DBConnections.getConnection()) {
            String sql = "delete from hoadontraxe where mahoadon = ?";
            PreparedStatement ps = conn.prepareStatement(sql);
